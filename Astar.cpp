...
/*	THIS IS FOR PSEUDO-NO_DIAG_MOV
	...
	AN OBSTACLE IS ADJACENT TO EXPANDING NODE
	AND THE NODE YOU CHECK: 	*/
#define HALF_DIAG_MOV true

/*	FOR CHECKING IN MAP IF NODE PASSABLE:	*/
#define INDEX_FOR_OBSTACLE 1
...
/*	
	...
	A-STAR:
	========
	Astar() will return the cost of the found optimal path
	or -1 if no path found,
	"path" will be a pointer to the first node in found optimal path,
	"main_closed" will point to the first node in a list of closed nodes:
		*/
int Astar( int **map, const int map_height, const int map_width, int start_pos_x, int start_pos_y, int finish_pos_x, int finish_pos_y, node **path, node **main_closed ) {
	...
	int done = 0;
	/*	This will hold the cumulated costs for the found optimal path,
		if no path found (no way from start to finish) it's == -1:	*/
	int cum_cost_path = -1;
	...
	while(  !done  ) {
		...
		if( current_node->x == finish_pos_x && current_node->y == finish_pos_y ) {
			...
			cum_cost_path = current_node->cost;
		...
		for( i = -1; i < 2; i++ ) {
			for( j = -1; j < 2; j++ ) {
				...
				if( current_node->x + i < 0 || current_node->x + i >= map_width || current_node->y + j < 0 || current_node->y + j >= map_height)
					continue;
				if( map[current_node->y + j][current_node->x + i] == INDEX_FOR_OBSTACLE )
					continue;
				...
				if ( i != 0 && j != 0 && HALF_DIAG_MOV ) {
					if( map[current_node->y + 0][current_node->x + i] == INDEX_FOR_OBSTACLE )
						continue;
					if( map[current_node->y + j][current_node->x + 0] == INDEX_FOR_OBSTACLE )
						continue;
				}
